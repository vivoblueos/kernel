...
pub struct Thread {
    ...
    // Futex word used by signal-wait syscalls (sigsuspend/sigtimedwait) to sleep
    // and by signal delivery to wake only the target thread.
    signal_wait_seq: AtomicUsize,
...
impl Thread {
    ...
    const fn const_new(kind: ThreadKind) -> Self {
        Self {
            ...
            signal_context: None,
            signal_wait_seq: AtomicUsize::new(0),
            blocked_mask: 0,
            saved_mask: 0,
        }
    }

    // Address of the per-thread futex word used by signal wait/wake paths.
    #[inline]
    pub fn signal_wait_seq_ptr(&self) -> &AtomicUsize {
        &self.signal_wait_seq
    }

    pub fn bump_signal_wait_seq(&self) -> usize {
        self.signal_wait_seq.fetch_add(1, Ordering::Release)
    }
