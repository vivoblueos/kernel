...
fn deliver_to_tid(tid: pid_t, sig: c_int, info: *mut siginfo_t) -> c_int {
    ...
    // Notify only the target thread waiting in sigsuspend/sigtimedwait.
    // NOTE: posix required choose one in thread group if multiple threads are waiting.
    target.bump_signal_wait_seq();
    let _ = futex::atomic_wake(target.signal_wait_seq_ptr(), 1);
...
pub fn sigsuspend(set: *const sigset_t) -> c_int {
    ...
    loop {
        ...
        let wait_word = t.signal_wait_seq_ptr().load(Ordering::Acquire);
        let _ = futex::atomic_wait(t.signal_wait_seq_ptr(), wait_word, None);
    }
...
pub fn sigtimedwait(set: *const sigset_t, info: *mut c_void, timeout: *const timespec) -> c_int {
    ...
    loop {
        ...
        // Otherwise, wait with optional timeout.
        let wait_word = t.signal_wait_seq_ptr().load(Ordering::Acquire);
        let res = futex::atomic_wait(t.signal_wait_seq_ptr(), wait_word, timeout_ticks);
        if res.is_err() {
            // Timeout.
            {
                let mut l = t.lock();
