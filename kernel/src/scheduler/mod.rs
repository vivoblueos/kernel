...
mod idle;
pub use idle::{get_idle_thread, set_idle_hook, IdleHook};
...
pub(crate) use wait_queue::{DefaultWaitQueueGuardDropper, OffsetOfWait, WaitEntry};
...
pub(crate) extern "C" fn save_context_finish_hook(hook: Option<&mut ContextSwitchHookHolder>) {
    ...
    };
    {
        ...
        crate::trace!(
            "finish_hook Switching from 0x{:x}: {{ SP: 0x{:x} PRI: {} }} to 0x{:x}: {{ SP: 0x{:x} PRI: {} }}",
            Thread::id(&old),
            old.saved_sp(),
            old.priority(),
            Thread::id(&next),
    ...
    if let Some(mut t) = retiring_thread {
        ...
        // when retiring, we need to ensure the cleanup is only called once
...
pub fn handle_tick_increment(elapsed_ticks: usize) -> bool {
    #[cfg(robin_scheduler)]
    {
