...
/// Provides initialized, independently owned state for raw buddy allocator tests.
///
...
impl BuddyTestArena {
    ...
    fn ptr_for_phys(&self, phys_addr: usize) -> *mut u8 {
        assert!(phys_addr >= self.data_phys_start());
        assert!(phys_addr < self.phys_end());
        let ptr = kernel_phys_to_virt(phys_addr) as *mut u8;
        let arena_start = self.backing.ptr().as_ptr() as usize;
        assert!((ptr as usize) >= arena_start);
        assert!((ptr as usize) < arena_start + self.backing.layout.size());
        ptr
    }
}
...
fn wrapper_reads_and_writes_real_backing_pages() {
    ...
    let first = arena.ptr_for_phys(first_phys);
    let block = arena.ptr_for_phys(block_phys);

    assert!(first_phys >= arena.data_phys_start());
    ...
    for worker in 0..2 {
        ...
        crate::thread::spawn(move || {
            ...
            for iteration in 0..32 {
                ...
                let ptr = arena.ptr_for_phys(phys);
