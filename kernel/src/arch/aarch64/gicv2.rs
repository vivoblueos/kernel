...
pub fn enable_irq_with_priority(irq: IrqNumber, cpu_id: usize, priority: Priority) {
    ...
    // Route SPIs (>= 32) to the target CPU. PPIs/SGIs are per-CPU and not routable via ITARGETSR.
    let intid: u32 = u32::from(irq);
    if intid >= 32 {
        gic.route_interrupt_to_cpu(intid, cpu_id as u8);
    }
}
