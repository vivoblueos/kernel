...
use libc::{
    AF_INET, ENOSYS, O_CREAT, O_DIRECTORY, O_RDONLY, O_RDWR, O_TRUNC, O_WRONLY, SEEK_END,
    SEEK_SET,
};
...
#[test]
fn test_framebuffer_devfs_read_write_seek() {
    let fd = open(c"/dev/fb0".as_ptr() as *const c_char, O_RDWR, 0);
    assert!(fd >= 0, "[VFS Test framebuffer]: Failed to open /dev/fb0");
    ...
    assert_eq!(read_size, read_buf.len() as isize);
    assert_eq!(read_buf, test_data);

    let end_offset = lseek(fd, 0, SEEK_END);
    assert!(end_offset > 0);
...
#[test]
fn test_framebuffer_devfs_ioctls() {
    let fd = open(c"/dev/fb0".as_ptr() as *const c_char, O_RDWR, 0);
    assert!(fd >= 0, "[VFS Test framebuffer]: Failed to open /dev/fb0");

    let mut fixed_info = unsafe { mem::zeroed::<libc::fb_fix_screeninfo>() };
    assert_eq!(
        ioctl(
            fd,
    ...
    assert!(fixed_info.smem_len > 0);
    assert!(fixed_info.line_length > 0);

    let mut variable_info = unsafe { mem::zeroed::<libc::fb_var_screeninfo>() };
    assert_eq!(
        ioctl(
            fd,
    ...
    assert!(variable_info.xres > 0);
    assert!(variable_info.yres > 0);
    assert!(variable_info.bits_per_pixel > 0);
    ...
    let mut unsupported_info = variable_info;
    unsupported_info.bits_per_pixel += 1;
    assert_eq!(
        ioctl(
            fd,
            libc::FBIOPUT_VSCREENINFO.into(),
    ...
    assert_eq!(ioctl(fd, 0xffff_ffff, core::ptr::null_mut()), -libc::EIO);
