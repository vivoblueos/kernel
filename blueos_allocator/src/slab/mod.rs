...
use crate::block::{used_block_hdr_for_allocation_unknown_align, BlockHdr, SIZE_USED};
use crate::tlsf;
use crate::kprintln;
use blueos_infra::{
...
mod dynamic_tests {
    ...
    fn dynamic_slab_realloc_grow() {
        let ptr = crate::malloc(16);
        assert!(!ptr.is_null());
        let ptr2 = crate::realloc(ptr, 256);
        assert!(!ptr2.is_null());
        crate::free(ptr2);
    }
    ...
    fn dynamic_slab_realloc_shrink_in_place() {
        let ptr = crate::malloc(64);
        assert!(!ptr.is_null());
        let ptr2 = crate::realloc(ptr, 32);
        assert!(!ptr2.is_null());
        crate::free(ptr2);
    }
    ...
    fn slab_used_returns_to_baseline_after_full_drain() {
        ...
        let baseline = crate::memory_info().used;

        for _ in 0..count {
            let p = crate::malloc(64);
            assert!(!p.is_null());
            ptrs.push(p); // no realloc: capacity pre-allocated
        }

        // allocated must have grown by exactly count * 64.
        let after_alloc = crate::memory_info().used;
        ...
        assert_eq!(crate::memory_info().used, baseline);
    }
    ...
    fn page_pool_reuses_pages_across_fill_drain_cycles() {
        ...
        let v1: Vec<*mut u8> = (0..bpp).map(|_| crate::malloc(128)).collect();
        for &p in &v1 {
            assert!(!p.is_null());
        }
        for p in v1 {
        ...
        let after_free = crate::memory_info().used;
        assert!(
        ...
        let baseline2 = crate::memory_info().used;
        for _ in 0..bpp {
            v2.push(crate::malloc(128));
        }
        for &p in &v2 {
            assert!(!p.is_null(), "pool reuse alloc must not fail");
        }
        assert_eq!(
            crate::memory_info().used - baseline2,
            bpp * 128,
        ...
        let after_free2 = crate::memory_info().used;
        assert!(
