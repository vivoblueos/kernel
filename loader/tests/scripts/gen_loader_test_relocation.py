...
def main():
    ...
    if any(value is not None
           for value in generic_region) and any(value is None
                                                for value in generic_region):
    ...
    for region in ("irom", "rodata", "rwdata"):
        ...
        if any(value is not None
               for value in provided) and any(value is None
    ...
    if args.region is not None:
        replacements["@REGION@"] = validate_identifier(args.region,
                                                       "region name")
