...
mod loader_test_config {
    ...
    use {
        ...
        librs::c_str::CStr,
        librs::esp32_flash::{
            EraseRangeRequest, MapDromRequest, MapExecRequest, ESP32_FLASH_ERASE_RANGE,
            ESP32_FLASH_MAP_DROM, ESP32_FLASH_MAP_EXEC, ESP32_FLASH_UNMAP, FLASH_IOCTL_ABI_VERSION,
        },
    ...
    pub fn map_written_flash(require_rodata: bool) -> loader::Result {
        ...
        let mut exec_request = MapExecRequest {
            version: FLASH_IOCTL_ABI_VERSION,
            size: core::mem::size_of::<MapExecRequest>() as u32,
            flags: 0,
            region_offset: IROM_FLASH_OFFSET as u32,
            image_size: irom_written as u32,
            mapped_address: 0,
        };
        flash_ioctl(
            ESP32_FLASH_MAP_EXEC,
            &mut exec_request as *mut MapExecRequest as usize,
        )?;
        if exec_request.mapped_address as usize != IROM_START {
            let _ = flash_ioctl(ESP32_FLASH_UNMAP, 0);
            return Err("Flash driver returned an unexpected XIP address");
        }

        if require_rodata {
            let mut drom_request = MapDromRequest {
                version: FLASH_IOCTL_ABI_VERSION,
                size: core::mem::size_of::<MapDromRequest>() as u32,
                flags: 0,
                region_offset: RODATA_FLASH_OFFSET as u32,
                image_size: rodata_written as u32,
                drom_vaddr: RODATA_START as u32,
                mapped_address: 0,
            };
            if flash_ioctl(
                ESP32_FLASH_MAP_DROM,
                &mut drom_request as *mut MapDromRequest as usize,
            )
            .is_err()
            {
                let _ = flash_ioctl(ESP32_FLASH_UNMAP, 0);
                return Err("Failed to map flash-backed read-only data");
            }
            if drom_request.mapped_address as usize != RODATA_START {
                let _ = flash_ioctl(ESP32_FLASH_UNMAP, 0);
                return Err("Flash driver returned an unexpected DROM address");
            }
        }
        Ok(())
    }
...
mod test_elf_loader {
    ...
    fn test_load_uselibrs_and_run_from_flash() {
        ...
        let mut mapper =
            loader::MemoryMapper::with_writer(Some(&TEST_REGIONS), Some(write_load_data));
        assert!(loader::load_elf(&buf, &mut mapper).is_ok());
