...
pub extern "C" fn current_cpu_id() -> usize {
    // Use tock_registers::interfaces::Readable for MPIDR_EL1
    let x: u64;
    unsafe { core::arch::asm!("mrs {}, mpidr_el1", out(reg) x, options(nostack)) };
    (x & 0xff) as usize
}
...
pub extern "C" fn send_ipi(target_core_id: usize) {
    let aff1 = 0u64;
    let aff2 = 0u64;
    let aff3 = 0u64;
