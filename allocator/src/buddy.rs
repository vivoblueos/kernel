...
#[repr(C)]
struct Page {
    list_node: IlistHead<Page, PageListNodeAdapter>,
    pfn: usize,
    order: u8,
    state: PageState,
}
...
//   |    |  page attributes: PageState::{Unavailable, Allocated, Free}  |
//   |    |  and order                                                   |
//   |    +--------------------------------------------------------------+
//   |
//   +--> manageable_start (first PFN managed by the buddy allocator)
//        |
//        |  Usable region is split into the largest possible buddy-aligned
//        |  blocks (up to MAX_ORDER=11, 8 MB). The head page of each block
//        |  has state=Free and its actual order; all other pages have
//        |  state=Unavailable and order=0. The first block's order may be
//        |  < MAX_ORDER if manageable_start is not MAX_ORDER-aligned.
//        |
//        |  +---------------+  +---------------+       +---------------+  +---------------+
//        |  | pfn=N         |  | pfn=N+1       |  ...  | pfn=M         |  | pfn=M+1       |  ...
//        |  | Free          |  | Unavailable   |       | Free          |  | Unavailable   |
//        |  | order=11      |  | order=0       |       | order=11      |  | order=0       |
//        |  +---------------+  +---------------+       +---------------+  +---------------+
//        |       ^ chunk 0 head                         ^ chunk 1 head
//        |
//        |           ... all pages in between state=Unavailable ...
//        |
//        |  +---------------+         +---------------+
//        |  | pfn=P         |  ...    | pfn=end       |
//        |  | Free          |         | Unavailable   |
...
//             has state=Free and a block order. An allocated block's head has
//             state=Allocated and retains its allocation order.
//
...
impl BuddyAllocator {
    ...
    pub fn metadata_layout(total_pages: usize) -> Result<Layout, BuddyInitError> {
        ...
        Layout::array::<Page>(total_pages).map_err(|_| BuddyInitError::MetadataLayoutOverflow)
    }
    ...
    pub unsafe fn init(
        ...
        while pfn < self.manageable_end {
            ...
            let page = &mut *self.pfn_to_page(pfn);
    ...
    fn alloc_pages_from(&mut self, order: usize, search_order: usize) -> Option<PageFrame> {
        ...
        for candidate_order in search_order..=MAX_ORDER {
            ...
            let page_ptr = self.pfn_to_page(pfn);
            ...
            while current_order > order {
                ...
                let buddy = unsafe { &mut *self.pfn_to_page(buddy_pfn) };
    ...
    pub unsafe fn free_pages(&mut self, frame: PageFrame, order: usize) {
        ...
        let mut current_page = &mut *self.pfn_to_page(pfn);
        debug_assert_eq!(
            current_page.state,
            PageState::Allocated,
        ...
        while current_order < MAX_ORDER {
            ...
            let buddy = &mut *self.pfn_to_page(buddy_pfn);
